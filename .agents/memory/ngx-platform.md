...
- Google OAuth: `GET /api/auth/google` → Google → `GET /api/auth/google/callback` → `findOrCreateGoogleUser()` → redirect to `/auth/callback?accessToken=...&refreshToken=...`. Credentials (`google_client_id`, `google_client_secret`) read from env vars first, then `dev_api_keys` settings. Developer panel shows the correct redirect URI to register in Google Console. New accounts get phone `+000000000000` placeholder.
